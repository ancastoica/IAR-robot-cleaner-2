...
namespace cleaner {
	montecarlo::montecarlo(world const &w, double epsilon, double gamma, double learning_rate, int episodes) : w(w),
																											   epsilon(epsilon),
																											   gamma(gamma),
																											   learning_rate(
																													   learning_rate),
	...
	double montecarlo::getFeatValueAt(int s) {
		double value = MIN;
		for (int a = 0; a < action::END; ++a) {
	...
	void montecarlo::featureBackup(int s, int a) {
		for (int feat = 0; feat < w.featuresnb; feat++) {
			this->feat_qf[s][a] = this->feat_qf[s][a] + w.features(w.getState(s))[feat * action::END + a] *
														this->theta[feat * action::END + a];
		}
	}
