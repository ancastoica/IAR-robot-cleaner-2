...
namespace cleaner {
	...
	public:
		...
		std::vector<int> features(state *const);
		...
		*/
		friend std::ostream &operator<<(std::ostream &os, const world &model) {
			...
			os << "\033[1m\033[32m ######################## \t\t<states>" << model.states.size() << "</states> \033[0m"
			   << std::endl;
			os << "\033[1m\033[32m ######################## \t\t<sound> " << model.sound(0.001) << "</sound> \033[0m"
			   << std::endl;
			os << "\033[1m\033[32m ######################## \t\t<actions> " << action::END - action::LEFT
			   << "</actions> \033[0m" << std::endl;
			os << "\033[1m\033[32m ######################## \t</preamble> \033[0m" << std::endl;
